...
class ChatDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.textView4.text = toUser.nama
        ...
        binding.send.setOnClickListener {
            ...
            rootRef!!.collection("contact").document(fromUid!!).collection("userContact")
                .document(fromUid!!).set(toUser, SetOptions.merge())
            rootRef!!.collection("rooms").document(fromUid!!).collection("userRooms")
                .document(roomId).set(fromUser, SetOptions.merge())
